// Parse the client-render probe configuration from URL query params (#166).
//
 ...
 *
 * The `engine` join key is fixed to `three` here (not URL-derived) so a three
 * run can never be mislabelled; the babylon probe (#167) fixes it to `babylon`.
 */
