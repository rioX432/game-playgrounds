// Client-render measurement probe (#166) — the three.js consumer of the shared
// #165 sidecar contract (`net-protocol` → `aggregateRenderWindow`).
//
// Responsibilities, kept deliberately small and rAF-free so they unit-test
// headless:
...
//   - feed a HUD fps-EMA into the sampler — only raw `dtMs` go in;
//   - read a clock itself — the caller (the rAF loop) supplies `nowMs`, so the
...
/** This probe runs inside the three.js client; its samples are always `three`. */
export const PROBE_ENGINE: Engine = "three";
 ...
 *
 * No clock reads, no rAF — same inputs always produce the same output, which is
 * exactly the part #166 unit-tests headless.
 */
export function buildClientRenderSample(
  ...
  return {
    ...
    // Web path always observes deltas via requestAnimationFrame (#165 parity gap).
...
/**
 * Stateful batching glue between the rAF loop and the pure sampler. The caller
 * pushes raw rAF timestamps via {@link recordFrame}; the probe computes raw
 * per-frame deltas, excludes warmup, closes fixed wall-clock windows, and emits
 * a `ClientRenderSample` per kept window. It is rAF-free (the caller supplies
 * the timestamp), so it is fully unit-testable without a browser.
 */
export class RenderProbe {
  ...
  /**
   * Feed one rAF timestamp. Computes the raw delta from the previous frame,
   * skips warmup, accumulates into the live window, and closes it when the
