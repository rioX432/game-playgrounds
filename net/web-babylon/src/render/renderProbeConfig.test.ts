...
describe("parseRenderProbeParams", () => {
  ...
  it("enables the probe with the documented defaults on probe=1", () => {
    ...
    expect(params.keys.engine).toBe("babylon");
  ...
  it("fixes the engine to babylon even if the URL tries to override it", () => {
    // engine is not a recognised param, so it can never be spoofed.
    const params = parseRenderProbeParams("?probe=1&engine=three");
    expect(params.keys.engine).toBe("babylon");
  });
