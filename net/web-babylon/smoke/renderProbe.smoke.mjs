// Playwright smoke for the client-render probe (#167).
//
// Loads the BUILT Babylon.js client against an ALREADY-RUNNING loaded bot room
// and a static preview server, lets the in-page probe collect its windows, then
// harvests `window.__clientRenderSamples` and writes them to `client-render.jsonl`
// from the Node side. This is auto-measurement, explicitly allowed by #167.
//
// HONEST CAVEAT: headless Chromium renders WebGL through SwiftShader (software),
// NOT the real GPU. The SHAPE and the pipeline (raw per-frame dt → shared sampler
...
//   cd net/web-babylon && PREVIEW_URL=http://localhost:4173 \
