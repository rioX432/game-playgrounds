# Sample Catalog — three-playground
...
`../babylon` and `../bevy`. For the cross-engine findings, see
[`../../COMPARISON.md`](../../COMPARISON.md).

Each sample lives in `src/samples/<id>/` with its own `README.md`
(*What it demonstrates / Controls / Feel & difficulty notes / Three.js gotchas*).
...
| 02 | Physics Grab & Throw | [`02-physics-grab-throw`](../src/samples/02-physics-grab-throw) | Rapier: grab the body under the crosshair, hold at distance, throw by impulse |
| 03 | Paint on Mesh | [`03-paint-on-mesh`](../src/samples/03-paint-on-mesh) | Paint onto a `CanvasTexture` via the hit UV (the disguise core) |
...
| 07 | Ragdoll | [`07-ragdoll-core`](../src/samples/07-ragdoll-core) | 11-capsule humanoid on physics joints; click a limb to punch, R to reset |
...
- `scene.ts` — ground, box grid, light preset.
